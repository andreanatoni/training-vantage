...
def resolve_phase_adjustment(config, phase, profile_name):
    """
    Resolve optional phase-based overrides from config.

    Supported schema:
    phase_adjustments.<phase>.<day_profile|default|all>
    with optional keys:
      - deficit_pct_delta
      - deficit_pct_override
      - training_cost_kcal_delta
      - ea_hard_floor_override
      - ea_target_min_override
      - note
    """
    if not phase:
        return None

    phase_adjustments = config.get("phase_adjustments", {})
    if not isinstance(phase_adjustments, dict):
        return None

    phase_cfg = phase_adjustments.get(phase)
    if not isinstance(phase_cfg, dict):
        return None

    for key in (profile_name, "default", "all"):
        candidate = phase_cfg.get(key)
...
def get_plan_adjustment(
    ...
    phase_adj = resolve_phase_adjustment(config, phase, profile_name)
    if phase_adj:
        if "deficit_pct_override" in phase_adj:
            deficit_pct = float(phase_adj["deficit_pct_override"])
        if "deficit_pct_delta" in phase_adj:
            deficit_pct += float(phase_adj["deficit_pct_delta"])
        if "training_cost_kcal_delta" in phase_adj:
            training_cost_kcal += int(round(float(phase_adj["training_cost_kcal_delta"])))
        if "ea_hard_floor_override" in phase_adj:
            ea_hard_floor = float(phase_adj["ea_hard_floor_override"])
        if "ea_target_min_override" in phase_adj:
            ea_target_min = float(phase_adj["ea_target_min_override"])

    deficit_pct = max(0.0, min(0.35, deficit_pct))
    training_cost_kcal = max(0, int(training_cost_kcal))
...
def generate_plan_markdown(category, plan_data, current_bmr, engine_meta=None):
    ...
    if engine_meta:
        ...
        phase_adj = engine_meta.get("phase_adjustment")
        if phase_adj:
            lines.append(
                "Phase Adjustment: {phase}/{scope} ({note})".format(
                    phase=phase_adj.get("phase"),
                    scope=phase_adj.get("scope"),
                    note=phase_adj.get("note", "applied"),
                )
            )
...
def generate_plan(category, silent=False, engine_overrides=None, output_md_file=None, output_json_file=None):
    ...
    engine_meta = {
        ...
        'phase_adjustment': adjustment.get('phase_adjustment'),
    }
...
def generate_week_plan(iso_week):
        ...
        day_outputs.append(
            {
                ...
                "phase": row.get("phase"),
                "workout_label": row.get("workout_label"),
                "md_file": str(dst_md.relative_to(REPO_ROOT)),
                "json_file": str(dst_json.relative_to(REPO_ROOT)),
            }
        )
    ...
    summary_json = {
        "meta": {
            ...
            "iso_week": iso_week,
            "source_running_plan": str(RUNNING_PLAN_FILE.relative_to(REPO_ROOT)),
        },
        "week": {
            "iso_week": iso_week,
